...
export function Login() {
  ...
  const loginMutation = useMutation({
    mutationFn: async ({ email, password, type }: { email: string; password: string; type: LoginType }) => {
  ...
  const handleGoogleLogin = () => {
    window.location.href = `/api/auth/google?type=${loginType}`;
  };
        ...
        <Card className="glassmorphism border-0 shadow-xl">
          <CardHeader className="text-center pb-6">
            ...
            <CardTitle className="text-2xl font-outfit font-bold bg-gradient-to-r from-pink-600 to-purple-600 bg-clip-text text-transparent">
              {loginType === 'admin' ? 'Painel Administrativo' : 'Entrar na Karooma'}
            </CardTitle>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex rounded-lg border p-1 bg-gray-50">
              <Button
                ...
                size="sm"
                className={`flex-1 ${loginType === 'user' 
                  ? 'bg-white shadow-sm' 
                  : 'hover:bg-gray-100'
                }`}
              ...
              <Button
                ...
                size="sm"
                className={`flex-1 ${loginType === 'admin' 
